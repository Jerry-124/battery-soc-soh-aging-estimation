...
def effective_resistance_from_segments(
    c1_discharge: dict, ocv_discharge: dict
) -> float:
    ...
    current_delta = abs(
        _median_current_a(c1_discharge) - _median_current_a(ocv_discharge)
    )
    ...
    if len(resistance) < 20:
        raise ValueError(
            "Insufficient valid points for effective resistance estimation"
        )
    return float(np.median(resistance))


def load_oxford_health_trajectory(
    path: str | Path, cell_number: int
    ...
    cell = loadmat(Path(path), variable_names=[cell_name], simplify_cells=True)[
        cell_name
    ]
    cycles: list[int] = []
    ...
    for key, characterization in cell.items():
        ...
        if (
            match is None
            or "C1dc" not in characterization
            or "OCVdc" not in characterization
        ):
            continue
        ...
        if not np.isfinite(capacity) or capacity <= 0:
            continue
        try:
            resistance = effective_resistance_from_segments(
                c1_discharge, characterization["OCVdc"]
            )
        except ValueError:
