...
class OCVCurve:
    ...
    def __call__(self, soc: np.ndarray | float) -> np.ndarray | float:
        ...
        value = np.interp(np.clip(z, self.soc[0], self.soc[-1]), self.soc, self.voltage_v) + self.voltage_bias_v
    ...
    def derivative(self, soc: np.ndarray | float) -> np.ndarray | float:
        ...
        z = np.asarray(soc, dtype=float)
        value = np.interp(np.clip(z, self.soc[0], self.soc[-1]), self.soc, slopes)
        return float(value) if value.ndim == 0 else value

    def with_bias(self, voltage_bias_v: float) -> "OCVCurve":
        return OCVCurve(self.soc.copy(), self.voltage_v.copy(), float(voltage_bias_v))
...
def read_arbin_workbook(path: str | Path) -> pd.DataFrame:
    ...
    frame = frame.drop_duplicates(subset="Data_Point").sort_values("Test_Time(s)").reset_index(drop=True)
...
def load_incremental_ocv_curve(path: str | Path) -> OCVCurve:
    ...
    for (cycle, step), group in data.groupby(["Cycle_Index", "Step_Index"], sort=False):
        duration = float(group["Test_Time(s)"].iloc[-1] - group["Test_Time(s)"].iloc[0])
        current_mean = float(group["Current(A)"].abs().mean())
        is_discharge_rest = (step == 4 and cycle == 1) or step == 6 or (step == 8 and cycle == 10)
        if is_discharge_rest and duration > 1000.0 and current_mean < 1e-3:
            discharged = float(group["Discharge_Capacity(Ah)"].iloc[-1])
            soc = float(np.clip(1.0 - discharged / capacity, 0.0, 1.0))
            voltage = float(group["Voltage(V)"].tail(min(60, len(group))).median())
            points.append((soc, voltage))
    if len(points) < 8:
        raise ValueError(f"Expected at least 8 relaxed discharge OCV points, found {len(points)}")
    points.sort()
    soc = np.array([point[0] for point in points])
    voltage = np.array([point[1] for point in points])
...
def load_dynamic_profile(path: str | Path, capacity_ah: float, dt_s: float = 1.0) -> MeasuredProfile:
    ...
    pre_charge = float(before["Charge_Capacity(Ah)"].max())
    ...
    arbin_current = np.interp(grid, source_time, dynamic["Current(A)"].to_numpy(float))
    current = -arbin_current  # CALCE/Arbin: positive charge; model: positive discharge.
    voltage = np.interp(grid, source_time, dynamic["Voltage(V)"].to_numpy(float))
