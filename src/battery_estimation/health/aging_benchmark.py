...
def pulse_anchored_fresh_parameters(
    capacity_ah: float, resistance_5s_ohm: float
) -> ECMParameters:
    ...
    response = (
        fractions[0]
        + fractions[1] * (1.0 - np.exp(-5.0 / tau1_s))
        + fractions[2] * (1.0 - np.exp(-5.0 / tau2_s))
    )
    total_r = resistance_5s_ohm / response
    r0, r1, r2 = fractions * total_r
    return ECMParameters(
        capacity_ah=capacity_ah, r0=r0, r1=r1, c1=tau1_s / r1, r2=r2, c2=tau2_s / r2
    )


def perturb_parameters(
    params: ECMParameters, capacity_multiplier: float, resistance_multiplier: float
) -> ECMParameters:
    return replace(
        params,
        capacity_ah=params.capacity_ah * capacity_multiplier,
        r0=params.r0 * resistance_multiplier,
        r1=params.r1 * resistance_multiplier,
        r2=params.r2 * resistance_multiplier,
    )


def run_kalman_observers(
    ...
    return {
        "ekf": ExtendedKalmanFilter(model, x0, p0.copy(), q, r).run(
            current_a, voltage_v
        )[:, 0],
        "ukf": UnscentedKalmanFilter(model, x0, p0.copy(), q, r).run(
            current_a, voltage_v
        )[:, 0],
    }
