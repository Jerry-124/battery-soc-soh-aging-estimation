...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Demonstrate pulse parameter identification on synthetic relaxation data"
    )
    parser.add_argument(
        "--output",
        type=Path,
        ...
        / "results"
        / "metrics"
        / "synthetic_parameter_identification.json",
    )
    ...
    polarization = current_step * (
        p.r1 * np.exp(-time_s / (p.r1 * p.c1)) + p.r2 * np.exp(-time_s / (p.r2 * p.c2))
    )
