...
def build_filter_inputs(
    config: dict, model: SecondOrderThevenin
...
def timed_run(
    name: str, estimator, current: np.ndarray, voltage: np.ndarray | None = None
):
...
def write_dataset(path: Path, dataset) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            ["time_s", "current_a", "voltage_v", "clean_voltage_v", "true_soc"]
        )
        writer.writerows(
            zip(
                dataset.time_s,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run a reproducible synthetic SOC/SOH benchmark"
    )
    ...
    aging = config["aging"]
    true_params = reference.aged(
        float(aging["capacity_factor"]), float(aging["resistance_factor"])
    )
    ...
    if config["observer"]["use_aging_adaptation"]:
        observer_params = adapt_parameters(
            reference, true_params.capacity_ah, float(aging["resistance_factor"])
        )
    observer_model = SecondOrderThevenin(observer_params, float(config["dt_s"]))
    x0, p0, q, r = build_filter_inputs(config, observer_model)
    cc = CoulombCounter(
        x0[0],
        observer_params.capacity_ah,
        observer_params.coulombic_efficiency,
        float(config["dt_s"]),
    )
    ...
    outputs = {}
    runtimes = {}
    for name, estimator, voltage in [
        ("coulomb_counting", cc, None),
        ("ekf", ekf, dataset.voltage_v),
        ("ukf", ukf, dataset.voltage_v),
    ]:
    ...
    for method, estimate in outputs.items():
        metrics[method] = calculate_metrics(
            dataset.true_soc, estimate, float(config["dt_s"])
        )
        metrics[method]["runtime_us_per_sample"] = runtimes[method]
    metrics["health"] = {
        "capacity_soh_pct": capacity_soh(
            true_params.capacity_ah, reference.capacity_ah
        ),
